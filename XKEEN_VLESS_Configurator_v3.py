...
class VlessOutboundGenerator:
    def __init__(self, root):
        self.root = root
        self.root.title("VLESS Outbound Generator для Xkeen - Полная версия с загрузкой")
        ...
        self.existing_outbounds_path = None
        self.existing_observatory_path = None
        
        self.create_widgets()
        
    def create_widgets(self):
        # Верхняя панель для загрузки существующих файлов
        load_frame = ttk.LabelFrame(self.root, text="Загрузить существующие файлы конфигурации", padding=10)
        load_frame.pack(fill="x", padx=10, pady=5)
        
        # 04_outbounds.json
        outbounds_frame = ttk.Frame(load_frame)
        outbounds_frame.pack(fill="x", pady=2)
        ttk.Label(outbounds_frame, text="04_outbounds.json:", width=20).pack(side="left")
        self.outbounds_path_var = tk.StringVar()
        ttk.Entry(outbounds_frame, textvariable=self.outbounds_path_var, width=50).pack(side="left", padx=5)
        ttk.Button(outbounds_frame, text="Обзор...", command=lambda: self.load_file("outbounds")).pack(side="left", padx=2)
        ttk.Button(outbounds_frame, text="Загрузить", command=self.load_outbounds_file).pack(side="left", padx=2)
        
        # 05_routing.json
        routing_frame = ttk.Frame(load_frame)
        routing_frame.pack(fill="x", pady=2)
        ttk.Label(routing_frame, text="05_routing.json:", width=20).pack(side="left")
        self.routing_path_var = tk.StringVar()
        ttk.Entry(routing_frame, textvariable=self.routing_path_var, width=50).pack(side="left", padx=5)
        ttk.Button(routing_frame, text="Обзор...", command=lambda: self.load_file("routing")).pack(side="left", padx=2)
        ttk.Button(routing_frame, text="Загрузить", command=self.load_routing_file).pack(side="left", padx=2)
        
        # 06_observatory.json
        observatory_frame = ttk.Frame(load_frame)
        observatory_frame.pack(fill="x", pady=2)
        ttk.Label(observatory_frame, text="06_observatory.json:", width=20).pack(side="left")
        self.observatory_path_var = tk.StringVar()
        ttk.Entry(observatory_frame, textvariable=self.observatory_path_var, width=50).pack(side="left", padx=5)
        ttk.Button(observatory_frame, text="Обзор...", command=lambda: self.load_file("observatory")).pack(side="left", padx=2)
        ttk.Button(observatory_frame, text="Загрузить", command=self.load_observatory_file).pack(side="left", padx=2)
        
        # Кнопка для загрузки всех файлов сразу
        ttk.Button(load_frame, text="Загрузить все файлы из папки", command=self.load_all_from_folder).pack(pady=5)
        ...
        info_text += "• 05_routing.json - балансировщик + принудительные правила\n"
        ...
        info_text += "💡 Загрузите существующие файлы для быстрого внесения изменений!"
    ...
    def load_file(self, file_type):
        ...
        file_path = filedialog.askopenfilename(
            title=f"Выберите файл",
            filetypes=[("JSON files", "*.json"), ("All files", "*.*")]
        )
        
        if file_path:
            if file_type == "outbounds":
                self.outbounds_path_var.set(file_path)
                self.existing_outbounds_path = file_path
            elif file_type == "routing":
                self.routing_path_var.set(file_path)
                self.existing_routing_path = file_path
            elif file_type == "observatory":
                self.observatory_path_var.set(file_path)
                self.existing_observatory_path = file_path
    
    def load_all_from_folder(self):
        """Загружает все файлы из выбранной папки"""
        folder_path = filedialog.askdirectory(title="Выберите папку с файлами конфигурации")
        
        if not folder_path:
            return
        
        # Ищем файлы
        outbounds_path = os.path.join(folder_path, "04_outbounds.json")
        routing_path = os.path.join(folder_path, "05_routing.json")
        observatory_path = os.path.join(folder_path, "06_observatory.json")
        
        loaded_count = 0
        
        # Загружаем outbounds
        if os.path.exists(outbounds_path):
            self.outbounds_path_var.set(outbounds_path)
            self.existing_outbounds_path = outbounds_path
            if self.load_outbounds_file(silent=True):
                loaded_count += 1
        
        # Загружаем routing
        if os.path.exists(routing_path):
            self.routing_path_var.set(routing_path)
            self.existing_routing_path = routing_path
            if self.load_routing_file(silent=True):
                loaded_count += 1
        
        # Загружаем observatory
        if os.path.exists(observatory_path):
            self.observatory_path_var.set(observatory_path)
            self.existing_observatory_path = observatory_path
            if self.load_observatory_file(silent=True):
                loaded_count += 1
        
        if loaded_count > 0:
            messagebox.showinfo("Успех", f"Загружено {loaded_count} файлов из папки:\n{folder_path}")
        else:
            messagebox.showwarning("Предупреждение", "В выбранной папке не найдено файлов конфигурации")
    
    def load_outbounds_file(self, silent=False):
        """Загружает существующий файл 04_outbounds.json"""
        if not self.existing_outbounds_path or not os.path.exists(self.existing_outbounds_path):
            if not silent:
                messagebox.showerror("Ошибка", "Файл не выбран или не существует")
            return False
        
        try:
            with open(self.existing_outbounds_path, 'r', encoding='utf-8') as f:
                # Удаляем комментарии
                content = ''
                for line in f:
            ...
            if 'outbounds' not in data:
                if not silent:
                    messagebox.showerror("Ошибка", "Неверный формат файла: отсутствует поле 'outbounds'")
                return False
            ...
            for outbound in data['outbounds']:
                ...
                # Пропускаем стандартные outbounds
                if tag in ['direct', 'block']:
                    continue
                
                # Пытаемся восстановить VLESS URL из конфига (для отображения)
                # В реальности мы не можем восстановить оригинальный URL, поэтому создаем заглушку
                vless_url = f"vless://{outbound['settings']['vnext'][0]['users'][0]['id']}@{outbound['settings']['vnext'][0]['address']}:{outbound['settings']['vnext'][0]['port']}"
                
                self.entries.append((tag, vless_url))
                self.tree.insert("", "end", values=(tag, vless_url[:60] + "..."))
                loaded_count += 1
            
            self.update_proxy_combo()
            
            if not silent:
                messagebox.showinfo("Успех", f"Загружено {loaded_count} прокси из файла")
            return True
    ...
    def load_routing_file(self, silent=False):
        """Загружает существующий файл 05_routing.json"""
        if not self.existing_routing_path or not os.path.exists(self.existing_routing_path):
            if not silent:
                messagebox.showerror("Ошибка", "Файл не выбран или не существует")
            return False
        ...
        try:
            ...
            # Очищаем текущие принудительные правила
            ...
            if not silent:
                total_rules = sum(len(r['domains']) + len(r['ips']) for r in self.forced_rules.values())
                messagebox.showinfo("Успех", f"Загружено {total_rules} принудительных правил из файла")
            return True
    ...
    def load_observatory_file(self, silent=False):
        """Загружает существующий файл 06_observatory.json"""
        if not self.existing_observatory_path or not os.path.exists(self.existing_observatory_path):
            if not silent:
                messagebox.showerror("Ошибка", "Файл не выбран или не существует")
            return False
        
        try:
            with open(self.existing_observatory_path, 'r', encoding='utf-8') as f:
                # Удаляем комментарии
                content = ''
                for line in f:
                    if not line.strip().startswith('//'):
            ...
            # Проверяем структуру, но ничего не загружаем, так как observatory генерируется автоматически
            if 'observatory' not in data:
                if not silent:
                    messagebox.showerror("Ошибка", "Неверный формат файла: отсутствует поле 'observatory'")
                return False
            
            # Просто показываем информацию о загруженном файле
            if not silent:
                subject_selector = data['observatory'].get('subjectSelector', [])
                messagebox.showinfo("Успех", f"Загружен файл observatory.json\nОтслеживаемые прокси: {', '.join(subject_selector)}")
            return True
    ...
    def clear_all(self):
        if messagebox.askyesno("Подтверждение", "Очистить весь список?"):
            ...
            self.outbounds_path_var.set("")
            self.routing_path_var.set("")
            self.observatory_path_var.set("")
            self.existing_outbounds_path = None
            self.existing_routing_path = None
            self.existing_observatory_path = None
    ...
    def generate_observatory_json(self, base_dir):
        ...
        # Если загружен существующий observatory, используем его настройки где возможно
        probe_interval = "60s"
        probe_url = "https://www.gstatic.com/generate_204"
        
        if self.existing_observatory_path and os.path.exists(self.existing_observatory_path):
            try:
                with open(self.existing_observatory_path, 'r', encoding='utf-8') as f:
                    content = ''
                    for line in f:
                    ...
                    if 'observatory' in data:
                        probe_interval = data['observatory'].get('probeInterval', probe_interval)
                        probe_url = data['observatory'].get('probeUrl', probe_url)
            except:
                pass  # Игнорируем ошибки, используем значения по умолчанию
    ...
    def generate_all_files(self):
        ...
        try:
            ...
            report += f"📄 06_observatory.json - мониторинг\n\n"
            ...
            if self.existing_outbounds_path:
                report += f"\n✅ Загружен outbounds.json: {os.path.basename(self.existing_outbounds_path)}"
            if self.existing_routing_path:
                report += f"\n✅ Загружен routing.json: {os.path.basename(self.existing_routing_path)}"
            if self.existing_observatory_path:
                report += f"\n✅ Загружен observatory.json: {os.path.basename(self.existing_observatory_path)}"
